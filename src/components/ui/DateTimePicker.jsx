...
const TriggerButton = forwardRef(function TriggerButton({ value, onClick }, ref) {
  ...
  return (
    ...
    >
      ...
      {/* Ícono lápiz cuando hay fecha — indica que se puede editar */}
      {hasDate ? (
        <svg
...
/**
 * @param {string}  value      - ISO local "YYYY-MM-DDTHH:mm"
 * @param {fn}      onChange   - recibe string ISO o ""
 * @param {boolean} autoOpen   - abre el calendario automáticamente al montar
 * @param {boolean} required
 */
export function DateTimePicker({ value, onChange, required, autoOpen }) {
  const pickerRef = useRef(null);

  useEffect(() => {
    if (!autoOpen) return;
    // Delay breve para que la animación de aparición del bloque termine
    const t = setTimeout(() => {
