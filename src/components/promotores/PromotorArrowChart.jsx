...
function ArrowConnectors({ formattedGraphicalItems }) {
  const lineItems = formattedGraphicalItems?.filter(
    (item) => item?.props?.points?.length > 1,
  );
...
export function PromotorArrowChart({ chartData, promotorNombre }) {
  return (
    <div>
      <p className="mb-3 text-sm text-neutral-600">
        Tendencia de <span className="font-bold text-brand">{promotorNombre}</span> — las
        flechas indican la evolución entre períodos (↑ sube, ↓ baja).
      </p>
