...
import { ButtonGroup, FormSection, RadioCard } from '../ui/ButtonGroup';
import { DateTimePicker } from '../ui/DateTimePicker';

const emptyReferido = () => ({ nombre: '', telefono: '' });
...
function desactivarReagenda(form) {
  return {
...
export function LeadModalForm({ lead, open, rolUsuario, onClose, onSave }) {
  const [form, setForm] = useState(() => buildInitialForm(lead));
  const [errorVenta, setErrorVenta] = useState('');

  const rol = rolUsuario === 'promotor' ? 'promotor' : 'supervisor';
  const productosDisponibles = useMemo(() => getProductosPorRol(rol), [rol]);

  useEffect(() => {
    if (open && lead) {
      const initial = buildInitialForm(lead);
      if (initial.idProducto && !puedeVenderProducto(rol, initial.idProducto)) {
        initial.idProducto = '';
      }
      setForm(initial);
      setErrorVenta('');
  ...
  const handleCanal = (canal) => patch({ canal });

  const handleReagendarToggle = (quiere) => {
    patch(quiere ? activarReagenda() : desactivarReagenda(form));
  };

  const handleEntrevista = (hubo) => {
    if (hubo) {
      patch({
        huboEntrevista: true,
        reagendarEntrevista: false,
  ...
  const handleGuardar = (e) => {
    ...
    if (form.resultadoEntrevista === 'compro') {
      ...
      if (!puedeVenderProducto(rol, form.idProducto)) {
        setErrorVenta('Tu rol no puede registrar la venta de ese producto.');
        return;
      }
      if (!form.estadoPago) {
        setErrorVenta('Indicá el estado del pago (seña o 100%).');
        return;
      }
    }
  ...
  const canalLabel =
    form.canal === 'llamada' ? 'llamada' : form.canal === 'mensaje' ? 'mensaje' : '';
      ...
      <Drawer.Portal>
        ...
        >
          ...
          >
              ...
              {showEntrevistaDetalle && (
                <div className="mt-2 space-y-2">
                  <RadioCard
                    ...
                    onChange={() =>
                      patch({ resultadoEntrevista: 'no_compro', idProducto: '', estadoPago: null })
                    }
                  />
                  <RadioCard
                    name="conEntrevista"
                    ...
                    <div className="space-y-5 rounded-xl border border-brand-100 bg-brand-50 p-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          {['sena', 'cien'].map((key) => {
                            const label = key === 'sena' ? 'Seña' : '100%';
                            const sel = form.estadoPago === key;
                            return (
                              <button
                                key={key}
                                type="button"
                                onClick={() => patch({ estadoPago: key })}
                                style={{ touchAction: 'manipulation' }}
                                className={`h-12 flex-1 rounded-lg border text-[15px] font-semibold transition-all duration-[140ms] ease-out ${
                                  sel
                                    ? 'border-brand-700 bg-brand-600 text-white active:bg-brand-700'
                                    : 'border-zinc-200 bg-white text-zinc-800 active:bg-brand-50 active:border-brand-600 active:text-brand-700'
                                }`}
            ...
            {/* Spacer para que el último campo no quede tapado por el footer sticky */}
