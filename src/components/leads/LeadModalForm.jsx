...
function activarReagenda(patch) {
  return {
...
function desactivarReagenda(patch) {
  return {
...
export function LeadModalForm({ lead, open, rolUsuario, onClose, onSave }) {
  ...
  useEffect(() => {
    if (open && lead) {
      ...
      if (
        initial.idProducto &&
        !puedeVenderProducto(rol, initial.idProducto)
      ) {
        initial.idProducto = '';
      }
      setForm(initial);
            ...
            {showSinEntrevista && (
              <div className="mt-3 space-y-2">
                ...
                <RadioOption
                  ...
                  onChange={() => patch({ ...activarReagenda(form), reagendarEntrevista: true })}
                />
                {showReagendaEnNo && (
                  ...
                  <div className="mt-3 space-y-4 rounded-xl border-2 border-brand/30 bg-brand-light p-3">
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                        <button
                          type="button"
                          onClick={() => patch({ estadoPago: 'sena' })}
                          className={`min-h-12 flex-1 rounded-full border-2 px-4 py-3 text-base font-bold touch-manipulation ${
                            form.estadoPago === 'sena'
                        ...
                        <button
                          type="button"
                          onClick={() => patch({ estadoPago: 'cien' })}
                          className={`min-h-12 flex-1 rounded-full border-2 px-4 py-3 text-base font-bold touch-manipulation ${
                            form.estadoPago === 'cien'
                              ? 'border-brand bg-brand text-white'
                              : 'border-neutral-300 bg-white text-neutral-900'
                          }`}
                        >
